...
| `config/fleet-manifest.md` | standing fleet context (issue #114): node registry (OS + lanes served) + the placement law (factory#60); injected into every dispatched task by the driver, overridable/extendable per launch with a live `DSH_FLEET_MANIFEST` snapshot |
...
| `.agents/skills/verify-before-dismissal/` | search the box-shared session history BEFORE declaring a limitation or "not supported" (issue #110 tools; hand-parser fallback) — a dismissal without a search is a guess |
